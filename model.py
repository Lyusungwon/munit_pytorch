...
class Content_encoder(nn.Module):
    def __init__(self, channel_size=64, content_code_h=54, content_code_w=44):
        ...
        self.conv = Sequential(
            ...
            nn.InstanceNorm2d(self.channel_size),
            nn.ReLU(inplace=True),
            nn.Conv2d(self.channel_size, self.channel_size * 2, 4, 2, 1),
            nn.InstanceNorm2d(self.channel_size * 2),
            nn.ReLU(inplace=True),
...
class Decoder(nn.Module):
    def __init__(self, output_h=218, output_w=178, channel_size=64, style_code_num=8):
        ...
        self.style_code_num = style_code_num
        self.mlp = Sequential(
            nn.Linear(self.style_code_num, self.channel_size),
            nn.ReLU(inplace=True),
            nn.Linear(self.channel_size, self.channel_size * 2),
            nn.ReLU(inplace=True),
            nn.Linear(self.channel_size * 2, self.channel_size * 2 * 4)
        )
        ...
        self.upconv = Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.LayerNorm([self.channel_size * 2, 108, 88]),
            nn.Upsample(scale_factor=2),
            nn.Conv2d(self.channel_size * 2, self.channel_size, 5, 1, 2),
            nn.ReLU(inplace=True),
            nn.LayerNorm([self.channel_size, 216, 176]),
            nn.Conv2d(self.channel_size, 3, 7, 1, 4)
        )

    def forward(self, c, s):
        s = self.mlp(s)
        s = s.view(-1, 2, self.channel_size * 4)
        c = self.res1(c, s[:, 0], s[:, 1])
        c = self.res2(c, s[:, 0], s[:, 1])
        c = self.res3(c, s[:, 0], s[:, 1])
        c = self.res4(c, s[:, 0], s[:, 1])
        x = self.upconv(c)
...
class Discriminator(nn.Module):
    def __init__(self, channel_size=1):
        ...
        self.conv1 = Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Conv2d(self.channel_size * 8, 1, (13, 11), 1)
        )
        self.conv2 = Sequential(
            ...
            nn.Conv2d(self.channel_size * 4, self.channel_size * 8, 4, 2, 1),
            nn.ReLU(inplace=True),
            nn.Conv2d(self.channel_size * 8, 1, (6, 5), 1)
        )
        self.conv3 = Sequential(
            ...
            nn.Conv2d(self.channel_size * 4, self.channel_size * 8, 4, 2, 1),
            nn.ReLU(inplace=True),
            nn.Conv2d(self.channel_size * 8, 1, (3, 2), 1)
        )
    def forward(self, x):
        x1 = self.conv1(x)
        x2 = self.conv2(x)
